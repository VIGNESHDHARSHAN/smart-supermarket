  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-2xl h-[92vh] max-h-[720px] rounded-3xl shadow-2xl border border-gray-200 dark:border-slate-800 flex flex-col overflow-hidden relative">
        ...
        {/* Chatbot Header */}
        <div className="bg-gradient-to-r from-primary-700 via-primary-600 to-emerald-700 dark:from-primary-950 dark:via-slate-900 dark:to-emerald-950 text-white p-3.5 sm:p-4 flex items-center justify-between border-b border-primary-500/20 shadow-md">
          <div className="flex items-center gap-3">
            <div 
              className="relative cursor-pointer group" 
              title="Click to interact with SmartMart AI Assistant"
              onClick={() => {
                if (!isAiSpeaking) {
                  soundEffects.playNotificationPing();
                  const greetingText = t('ai_greeting', 'Hello! I am your SmartMart AI Shopping Assistant. How can I help with your shopping today?');
                  setLiveCaption({
                    active: true,
                    type: 'ai',
                    text: greetingText,
            ...
            <div>
              ...
              <p className="text-xs text-primary-100/90 flex items-center gap-1.5 mt-0.5">
                ...
                <span>Multilingual Shopping Assistant • {currentLangMeta?.nativeName}</span>
              </p>
            </div>
          </div>
        ...
        {/* Live Caption & Modern AI Assistant Spotlight Stage */}
        {liveCaptionEnabled && liveCaption.active && (
          <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-emerald-950 text-white px-4 sm:px-5 py-3.5 border-b-2 border-emerald-500/40 shadow-2xl animate-in slide-in-from-top-3 duration-300 z-30 relative overflow-hidden">
            {/* Ambient Background Aura Glow */}
            <div className="absolute -top-12 -left-12 w-48 h-48 bg-emerald-500/20 rounded-full blur-2xl pointer-events-none"></div>
            <div className="absolute -bottom-12 -right-12 w-48 h-48 bg-teal-500/20 rounded-full blur-2xl pointer-events-none"></div>

            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3.5 relative z-10">
              
              {/* Modern AI Assistant Visualizer Stage */}
              <div className="flex-shrink-0 flex flex-col items-center">
                ...
                <span className="mt-1 text-[10px] font-bold text-emerald-300 px-2 py-0.5 rounded-full bg-emerald-950/60 border border-emerald-500/30">
                  {liveCaption.type === 'ai'
                    ? (isAiSpeaking ? 'Speaking' : 'Explaining')
                    : (isListening ? 'Listening' : 'Thinking')}
                </span>
              </div>

              {/* Speaking Speech Bubble Dialog with Captions */}
              <div className="flex-1 min-w-0 w-full bg-white/10 dark:bg-slate-900/90 backdrop-blur-md rounded-2xl p-3.5 sm:p-4 border border-emerald-400/30 shadow-inner relative">
                {/* Speech Bubble Arrow pointing toward Assistant */}
                <div className="hidden sm:block absolute top-6 -left-2.5 w-0 h-0 border-t-[8px] border-t-transparent border-b-[8px] border-b-transparent border-r-[10px] border-r-white/10 dark:border-r-slate-900/90"></div>
                ...
                <div className="flex items-center justify-between gap-2 mb-2 pb-2 border-b border-white/10">
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full flex items-center gap-1.5 ${
                      ...
                      {liveCaption.type === 'ai' ? 'SmartMart AI • Speaking' : 'Live User Microphone'}
                    </span>
                    ...
                    <div className="flex items-center gap-0.5 h-3">
                      ...
                      <span className="w-0.5 bg-teal-300 rounded-full animate-bounce h-2.5 delay-100"></span>
                    </div>
                  </div>
                    ...
                    {liveCaption.type === 'ai' && (
                      <button
                        ...
                        onClick={() => {
                          soundEffects.stopSpeaking();
                          setIsAiSpeaking(true);
                          soundEffects.speakText(liveCaption.text, currentLangMeta?.speechLang || 'en-US', {
                            onStart: () => setIsAiSpeaking(true),
                            onEnd: () => setIsAiSpeaking(false),
                            onError: () => setIsAiSpeaking(false)
                          });
                        }}
                        className="px-2 py-1 rounded-lg bg-emerald-500/20 hover:bg-emerald-500/30 text-[11px] font-bold text-yellow-300 flex items-center gap-1 border border-emerald-400/30 transition-all shadow-xs"
                ...
                {/* Spoken Text Headline */}
                <p className="text-xs sm:text-sm md:text-base font-semibold text-white leading-relaxed font-sans select-text drop-shadow-xs">
                  "{liveCaption.text}"
                  {liveCaption.isInterim && <span className="inline-block w-1.5 h-4 ml-1 bg-yellow-400 animate-pulse align-middle"></span>}
                </p>

                {/* Action Cue Subtitle */}
                <div className="mt-2.5 pt-1.5 border-t border-white/10 flex items-center justify-between text-[11px] text-emerald-300/90 font-medium">
                  <span className="flex items-center gap-1.5">
                    <Sparkles className="w-3 h-3 text-yellow-400 animate-spin" />
                    <span>
                      {isAiSpeaking 
                        ? 'Explaining aisles & items with voice guidance'
                        : isListening 
                        ? 'Listening closely to your voice prompt...'
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 bg-slate-50/70 dark:bg-slate-950/60">
          ...
          {isTyping && (
            <div className="flex items-center gap-3 ml-2 my-2 bg-primary-500/10 dark:bg-primary-950/40 p-2.5 px-4 rounded-2xl border border-primary-400/20 max-w-md animate-in fade-in shadow-xs">
              <div className="w-7 h-7 rounded-xl bg-gradient-to-tr from-primary-600 to-indigo-600 text-white flex items-center justify-center flex-shrink-0 shadow-xs">
                <Bot className="w-4 h-4 animate-pulse" />
              </div>
              <div className="text-xs text-primary-800 dark:text-primary-300 font-semibold space-y-0.5">
                <div className="flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-primary-500 animate-bounce"></span>
                  <span className="w-1.5 h-1.5 rounded-full bg-primary-500 animate-bounce delay-100"></span>
                  <span className="w-1.5 h-1.5 rounded-full bg-primary-500 animate-bounce delay-200"></span>
                  <span>SmartMart AI is thinking...</span>
                </div>
                <div className="text-[10px] text-gray-500 dark:text-gray-400 font-normal">
                  Checking store inventory, shelf aisles &amp; recipe ingredients
                </div>
              </div>
            </div>
          )}
        ...
        {showVoiceHub && (
          <div className="bg-gradient-to-br from-slate-900 via-primary-950 to-slate-900 border-t border-primary-500/30 p-3 sm:p-4 text-white animate-in slide-in-from-bottom-3 duration-200 z-20 shadow-2xl">
            ...
            <p className="text-[11px] text-primary-200/80 mb-2">
              Tap any spoken command below to speak with AI Assistant with full live captions & voice:
            </p>
