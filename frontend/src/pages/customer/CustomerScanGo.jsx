...
export default function CustomerScanGo() {
  ...
  const [isCameraTroubleshootOpen, setIsCameraTroubleshootOpen] = useState(false);
  const [capturedPhoto, setCapturedPhoto] = useState(null);
  const [isScanningPhoto, setIsScanningPhoto] = useState(false);
  const [photoMismatchModalOpen, setPhotoMismatchModalOpen] = useState(false);
  const [photoFilterSearch, setPhotoFilterSearch] = useState('');
  ...
  // Start Camera Stream with Mobile & Browser fallbacks
  const startCamera = async (overrideFacing) => {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      setCameraError(
        isHttp 
          ? 'Mobile browsers (Chrome / Safari) strictly block live continuous video over non-HTTPS Wi-Fi (192.168.x.x). Tap "Snap Photo with Phone Camera" to scan immediately with zero setup, or tap the Help guide.' 
          : 'Camera access is not supported on this browser context. Please use "Snap Photo with Phone Camera".'
      );
    ...
    if (!activeStream) {
      console.warn('Camera stream access failed:', lastErr);
      let errMsg = 'Could not access device camera.';
      if (lastErr?.name === 'NotAllowedError' || lastErr?.name === 'PermissionDeniedError') {
        errMsg = 'Camera permission was denied. Tap the lock/tune icon in your browser address bar and enable Camera access.';
        ...
        errMsg = 'Mobile browser security blocked live video on HTTP. Tap "Snap Photo with Phone Camera" or see Help guide.';
      }
  ...
  // Toggle Physical Camera Facing (Rear vs Front)
  ...
  // Native Phone Camera Capture & Photo Barcode Decoding
  const handleImageCapture = async (e) => {
    ...
    // 1. Try Native BarcodeDetector API (Android Chrome & modern Chromium browsers)
    if ('BarcodeDetector' in window) {
      try {
        const detector = new window.BarcodeDetector({ 
          formats: ['qr_code', 'ean_13', 'code_128', 'ean_8', 'upc_a', 'upc_e', 'code_39'] 
        });
        ...
        console.warn('Native barcode detect error:', err);
      }
    }

    // 2. Fallback to html5-qrcode dynamically for iOS Safari / cross-browser photo scanning
    if (!detectedBarcode) {
      try {
        ...
        if (window.Html5Qrcode) {
          const html5QrCode = new window.Html5Qrcode("html5-temp-scanner");
          const decodedText = await html5QrCode.scanFile(file, false);
          if (decodedText) {
            detectedBarcode = decodedText;
          }
          try { html5QrCode.clear(); } catch(e) {}
        }
      } catch (err) {
        console.warn('html5-qrcode scan error:', err);
      }
    }
    ...
    if (detectedBarcode) {
      const match = products.find(p => p.barcode === detectedBarcode || p.id === detectedBarcode);
      if (match) {
        ...
        try {
          const netRes = await fetchProductImageFromInternet(detectedBarcode, detectedBarcode, 'Groceries');
          if (netRes && netRes.imageUrl) {
            const autoProduct = {
              id: 'PRD-EXT-' + Date.now(),
              barcode: detectedBarcode,
              name: `Retail Item (${detectedBarcode})`,
              category: 'Groceries',
              price: 99,
              unit: '1 unit',
              aisle: 1,
    ...
    // If barcode not recognized or reading failed, open photo resolver modal
    setPhotoMismatchModalOpen(true);
    if (fileInputRef.current) fileInputRef.current.value = '';
  };
  ...
  useEffect(() => {
    ...
    if (scannerMode === 'camera') {
      ...
      if ('BarcodeDetector' in window) {
        try {
          const detector = new window.BarcodeDetector({ formats: ['qr_code', 'ean_13', 'code_128', 'ean_8', 'upc_a', 'upc_e'] });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <div className="bg-white p-1.5 rounded-2xl shadow-md border border-white/20 inline-flex items-center justify-center">
                <img src="/gosmart-icon.png" alt="GoSmart" className="h-10 w-10 object-contain" />
              </div>
              <div>
                <div className="inline-flex items-center gap-2 bg-purple-500/20 border border-purple-400/30 text-purple-300 text-xs font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className={`bg-charcoal-950 rounded-3xl overflow-hidden shadow-2xl border-4 transition-all duration-300 relative p-6 text-white ${
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 mb-4 pb-3 border-b border-charcoal-800 text-xs">
              <div className="flex items-center gap-2">
                <span className={`w-2.5 h-2.5 rounded-full ${cameraActive ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400'}`}></span>
                <span className="font-extrabold text-gray-200">
                  {scannerMode === 'camera' 
                    ? (cameraActive ? `Live Camera (${facingMode === 'environment' ? 'Rear' : 'Front'})` : 'Camera Scanner') 
                    : 'Simulated Optical HUD'}
                </span>
              </div>

              <div className="flex items-center gap-2 flex-wrap">
                {/* Direct Snap Photo Button for Phones */}
                <button
                  type="button"
                  onClick={() => fileInputRef.current?.click()}
                  className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-[11px] font-bold flex items-center gap-1.5 shadow-sm active:scale-95 transition-all"
                  title="Snap a photo of the barcode using phone camera (works on all phones)"
                >
                ...
                {scannerMode === 'camera' && (
                  <button
                    ...
                    className="px-2 py-1 bg-charcoal-800 hover:bg-charcoal-700 rounded-lg text-[11px] font-semibold text-gray-300 flex items-center gap-1 border border-charcoal-700"
                ...
                >
                  <span>{scannerMode === 'simulated' ? 'Switch to Camera' : 'Simulated'}</span>
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setIsCameraTroubleshootOpen(true)}
                  className="p-1.5 rounded-lg border border-charcoal-700 bg-charcoal-800 hover:bg-charcoal-700 text-gray-300 hover:text-white text-xs transition-colors"
                  title="Phone Camera Help & Troubleshooting"
                >
            ...
            <div className="relative aspect-[16/10] bg-black rounded-2xl overflow-hidden flex flex-col items-center justify-center border border-charcoal-700">
              ...
              {/* Hidden file input for native phone camera capture */}
              <input
                ref={fileInputRef}
                type="file"
                accept="image/*"
              ...
              />
              <div id="html5-temp-scanner" style={{ display: 'none' }} />
              ...
              <video 
                ref={videoRef} 
                className={`w-full h-full object-cover ${scannerMode === 'camera' && cameraActive ? 'block' : 'hidden'}`} 
                autoPlay 
                playsInline 
              ...
              {/* Inactive or Blocked Camera UI on Phone */}
              {scannerMode === 'camera' && !cameraActive && (
                <div className="absolute inset-0 bg-gradient-to-b from-charcoal-900/90 to-charcoal-950/95 flex flex-col items-center justify-center p-6 text-center z-10">
                  <div className="w-12 h-12 rounded-2xl bg-primary-500/10 border border-primary-500/30 flex items-center justify-center text-primary-400 mb-3 shadow-[0_0_20px_rgba(16,185,129,0.2)]">
                  ...
                  <p className="text-xs text-gray-300 max-w-sm mb-4 leading-relaxed">
                    {isHttpNetwork 
                      ? 'Mobile browsers block live continuous video over non-HTTPS Wi-Fi. Tap below to snap photos with your phone camera directly!' 
                      : cameraError || 'Tap below to activate camera permissions or snap a photo of any barcode.'}
                  </p>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-2.5 w-full max-w-xs">
                    <button
                      type="button"
                      onClick={() => fileInputRef.current?.click()}
                      className="w-full py-2.5 px-4 bg-gradient-to-r from-emerald-500 to-primary-600 hover:from-emerald-600 hover:to-primary-700 text-white rounded-xl text-xs font-extrabold shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2 active:scale-95 transition-all"
                    >
                    ...
                    <button
                      ...
                      onClick={() => startCamera()}
                      className="w-full sm:w-auto py-2 px-3 bg-charcoal-800 hover:bg-charcoal-700 text-gray-200 border border-charcoal-600 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition-all"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${isStartingCamera ? 'animate-spin' : ''}`} />
                      <span>{isStartingCamera ? 'Starting...' : 'Retry'}</span>
                    </button>
                  </div>
                  ...
                  <button
                    type="button"
                    onClick={() => setIsCameraTroubleshootOpen(true)}
                    className="mt-3 text-[11px] text-amber-400 hover:text-amber-300 flex items-center gap-1 underline underline-offset-2"
                  >
                    <HelpCircle className="w-3 h-3" />
                    <span>How to enable Live Stream on Phone</span>
                  </button>
                </div>
              )}

              {/* Simulated Scanner Mode */}
              {scannerMode !== 'camera' && (
              ...
              {/* Processing Snapped Photo Overlay */}
              {isScanningPhoto && (
                <div className="absolute inset-0 bg-black/80 backdrop-blur-xs flex flex-col items-center justify-center z-20">
                  <div className="w-10 h-10 rounded-full border-3 border-primary-500 border-t-transparent animate-spin mb-3"></div>
                  <div className="text-xs font-bold text-white">Analyzing Barcode from Photo...</div>
                  <div className="text-[10px] text-gray-400 mt-1">Processing captured image</div>
                </div>
              )}

              {/* Viewfinder Holographic Corner Targets (visible when camera active or simulated) */}
              ...
              <div className="absolute bottom-3 left-4 text-[9px] font-mono text-gray-400 bg-black/60 px-2 py-0.5 rounded border border-white/10">
                {cameraActive ? `STREAM: ACTIVE • ${facingMode.toUpperCase()}` : 'SENSOR: AI BARCODE SCANNER'}
              </div>

              {cameraError && cameraActive && (
                <div className="absolute top-3 inset-x-4 bg-red-900/80 border border-red-500/50 p-2 rounded-xl text-[10px] text-red-200 text-center z-10">
                  {cameraError}
                </div>
              )}
            </div>
                  ...
                  <div className="flex items-center justify-between border-b border-white/10 pb-3 text-xs">
                    <div className="flex items-center gap-2">
                      <div className="bg-white p-1 rounded-lg shadow-xs">
                        <img src="/gosmart-icon.png" alt="GoSmart" className="h-4 w-4 object-contain" />
                      </div>
                      <span className="font-extrabold text-purple-200">GOSMART SCAN &amp; GO EXIT PASS</span>
                    </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 border border-gray-200 animate-in fade-in zoom-in-95 font-mono text-xs">
            <div className="flex items-center justify-between border-b border-gray-200 pb-3 font-sans">
              <div className="flex items-center gap-2.5">
                <div className="bg-white p-1 rounded-lg border border-gray-100 shadow-2xs">
                  <img src="/gosmart-logo.png" alt="GoSmart" className="h-8 w-auto object-contain max-w-[110px]" />
                </div>
                <div>
                  <h4 className="font-extrabold text-sm text-gray-900 leading-tight">Tax Invoice Receipt</h4>
                  <p className="text-[10px] text-gray-500">GoSmart Supercenter #104</p>
                </div>
              </div>
      ...
      {/* Mobile Camera Help & Troubleshooting Modal */}
      {isCameraTroubleshootOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-charcoal-950 text-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-charcoal-800 space-y-4 max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-charcoal-800 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-extrabold text-sm text-white">Phone Camera Setup Guide</h3>
                  <p className="text-[11px] text-gray-400">How to open & scan barcodes on mobile</p>
                </div>
              </div>
              <button 
                onClick={() => setIsCameraTroubleshootOpen(false)}
                className="p-1.5 text-gray-400 hover:text-white rounded-lg hover:bg-charcoal-800"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3.5 text-xs text-gray-300">
              {/* Option 1: Zero Setup Photo Capture */}
              <div className="p-3.5 bg-emerald-950/40 border border-emerald-500/30 rounded-2xl space-y-1.5">
                <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs">
                  <CheckCircle2 className="w-4 h-4 flex-shrink-0" />
                  <span>Method 1: Snap Photo (Recommended • Zero Setup)</span>
                </div>
                <p className="text-[11px] text-gray-300 leading-relaxed">
                  Tap the green <strong>"Snap Photo with Phone Camera"</strong> button on the scanner. 
                  This immediately opens your phone's native camera on both iPhone and Android without requiring any HTTPS or Wi-Fi configuration!
                </p>
                <button
                  type="button"
                  onClick={() => {
                    setIsCameraTroubleshootOpen(false);
                    fileInputRef.current?.click();
                  }}
                  className="mt-1 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-[11px] font-bold flex items-center gap-1.5"
                >
                  <Camera className="w-3.5 h-3.5" />
              ...
              {/* Option 2: Live Video Stream on Android Chrome */}
              <div className="p-3.5 bg-charcoal-900 border border-charcoal-800 rounded-2xl space-y-2">
                <div className="flex items-center gap-2 text-amber-400 font-bold text-xs">
                  <Sparkles className="w-4 h-4 flex-shrink-0" />
                  <span>Method 2: Enable Live Video Stream (Chrome on Android)</span>
                </div>
                <p className="text-[11px] text-gray-300 leading-relaxed">
                  Mobile Chrome blocks continuous live WebRTC video when accessing local Wi-Fi IPs (<code className="bg-black/50 px-1 py-0.5 rounded text-gray-200">http://192.168.x.x</code>) because it is not HTTPS. You can easily enable it:
                </p>
                <ol className="list-decimal list-inside space-y-1 text-[11px] text-gray-400 pl-1">
                  <li>In Chrome on your phone, open a new tab and go to: <br /><code className="bg-black px-1.5 py-0.5 rounded text-amber-300 select-all font-mono text-[10px]">chrome://flags/#unsafely-treat-insecure-origin-as-secure</code></li>
                  <li>In the text field, paste your local network URL: <br /><code className="bg-black px-1.5 py-0.5 rounded text-emerald-300 select-all font-mono text-[10px]">{typeof window !== 'undefined' ? `${window.location.protocol}//${window.location.host}` : 'http://192.168.x.x:5173'}</code></li>
                  <li>Change the dropdown next to it from <strong>Disabled</strong> to <strong>Enabled</strong>.</li>
                  <li>Tap the blue <strong>Relaunch</strong> button at the bottom of Chrome.</li>
                </ol>
              </div>

              {/* Option 3: Browser Permissions */}
              <div className="p-3.5 bg-charcoal-900 border border-charcoal-800 rounded-2xl space-y-1.5">
                <div className="flex items-center gap-2 text-primary-400 font-bold text-xs">
                  <ShieldCheck className="w-4 h-4 flex-shrink-0" />
                  <span>Method 3: Reset Browser Camera Permissions</span>
                </div>
                <ul className="list-disc list-inside space-y-1 text-[11px] text-gray-400 pl-1">
                  <li><strong>iPhone (Safari):</strong> Tap the <em>"aA"</em> or Page Settings icon on the left of the address bar &rarr; Website Settings &rarr; Camera &rarr; select <strong>Allow</strong>.</li>
                  <li><strong>Android (Chrome):</strong> Tap the lock or tune icon on the left of the address bar &rarr; Permissions &rarr; Camera &rarr; select <strong>Allow</strong>.</li>
                </ul>
              </div>
            </div>
            ...
            <div className="pt-2">
              <Button
                className="w-full bg-primary-600 hover:bg-primary-700 text-white font-bold text-xs"
                onClick={() => setIsCameraTroubleshootOpen(false)}
              >
                Got It, Close Guide
              </Button>
      ...
      {/* Photo Barcode Resolver Modal (when a photo is taken but barcode wasn't auto-decoded) */}
      {photoMismatchModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-gray-200 space-y-4 max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="font-extrabold text-sm text-gray-900">Photo Captured!</h4>
                  <p className="text-[11px] text-gray-500">Confirm your product to add to cart</p>
                </div>
              </div>
              <button 
                onClick={() => {
                  setPhotoMismatchModalOpen(false);
                  setCapturedPhoto(null);
                }} 
            ...
            {capturedPhoto && (
              <div className="relative aspect-video rounded-2xl overflow-hidden bg-black border border-gray-200">
                <img src={capturedPhoto} alt="Captured barcode" className="w-full h-full object-contain" />
                <div className="absolute bottom-2 inset-x-2 bg-black/70 text-white px-2.5 py-1 rounded-lg text-[10px] text-center">
                  Photo saved • Tap your matching item below to add to basket
                </div>
              </div>
            )}

            {/* Quick Product Search & 1-Tap Add */}
            <div className="space-y-2">
              <div className="relative">
                <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
                <input
                  ...
                  value={photoFilterSearch}
                  onChange={(e) => setPhotoFilterSearch(e.target.value)}
                  className="w-full pl-8 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs text-gray-800 placeholder:text-gray-400"
                />
              </div>

              <div className="divide-y divide-gray-100 max-h-48 overflow-y-auto rounded-xl border border-gray-100">
                {products
                  .filter(p => !photoFilterSearch || p.name.toLowerCase().includes(photoFilterSearch.toLowerCase()) || p.barcode.includes(photoFilterSearch))
                  .slice(0, 6)
                  .map(product => (
                    <div 
                      key={product.id} 
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        <img src={product.image} alt={product.name} className="w-9 h-9 object-cover rounded-lg border border-gray-200" />
                        <div>
                          <div className="text-xs font-bold text-gray-900">{product.name}</div>
                          <div className="text-[10px] text-gray-500 font-mono">₹{product.price} • #{product.barcode}</div>
                        </div>
                      </div>
                      <Button
                        size="sm"
                        className="bg-primary-600 hover:bg-primary-700 text-white text-[10px] font-bold h-7 px-2.5"
                        onClick={() => {
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-gray-100">
              ...
              >
                <Camera className="w-3.5 h-3.5 mr-1" /> Retake Photo
              </Button>
              <Button
                variant="outline"
                className="flex-1 text-xs font-bold text-gray-600"
                onClick={() => {
                  setPhotoMismatchModalOpen(false);
                  setCapturedPhoto(null);
                }}
