...
export default function CustomerLogin() {
  ...
  useEffect(() => {
    const handleGoogleRedirect = async () => {
      const googleUser = await parseGoogleOAuthHash(false);
      if (googleUser) {
        try {
          const backendRes = await apiGoogleLogin(googleUser, false);
          loginCustomer(backendRes?.user || googleUser);
        } catch (e) {
  ...
  // Direct Google Identity Services (One Tap & Official Button)
  useEffect(() => {
    let mounted = true;
    initGoogleIdentityDirect({
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 transition-colors">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        <Link to="/customer" className="inline-flex justify-center hover:opacity-95 transition-opacity">
          <div className="bg-white dark:bg-white/95 p-3 rounded-2xl shadow-md border border-gray-200 dark:border-white/20 flex items-center justify-center">
        ...
        <h2 className="mt-4 text-3xl font-black text-gray-900 dark:text-white tracking-tight">
          Welcome to GoSmart Supermarket
        </h2>
        <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
          Sign in or create an account to unlock 15-min express delivery & Scan & Go self-checkout
        </p>
      </div>
